...
void WLGooseHeuristic::print_statistics() const {
  for (size_t i = 0; i < iterations_; i++) {
    log << wl_algorithm_ << " seen/unseen colours in itr " << i << ": " << cnt_seen_colours[i]
        << " " << cnt_unseen_colours[i] << std::endl;
  }
}
...
std::vector<int> WLGooseHeuristic::lwl2_feature(const CGraph &graph) {
  // feature to return is a histogram of colours seen during training
  std::vector<int> feature(feature_size_, 0);
      ...
      for (int u = 0; u < n_nodes; u++) {
        for (int v = u + 1; v < n_nodes; v++) {
          ...
          for (const int w : pair_to_neighbours[index]) {
            ...
            neighbour_colours.push_back(std::make_pair(std::min(cs1, cs2), std::max(cs1, cs2)));
          }
...
std::vector<int> WLGooseHeuristic::lwl3_feature(const CGraph &graph) {
  ...
  // const int n_triples = static_cast<int>((n_nodes * (n_nodes - 1) * (n_nodes - 2)) / 6);

  // // role of colours_0 and colours_1 is switched every iteration for storing old and new colours
  // std::vector<int> colours_0(n_triples);
  // std::vector<int> colours_1(n_triples);
  // std::vector<std::vector<std::pair<int, int>>> edges = graph.get_edges();
  ...
  //   }
  // }
  // std::vector<std::set<int>> triple_to_neighbours(n_triples, std::set<int>());
  // for (int u = 0; u < n_nodes; u++) {
  //   for (int v = u + 1; v < n_nodes; v++) {
