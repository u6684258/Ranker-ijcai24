...
do 
    ...
    do
        DESC=${domain}_${r}_${k}_${l}_${p}_${m}
        SAVE_FILE="trained_kernel_models/${DESC}.joblib"
        if [ ! -f $SAVE_FILE ]; then
            echo python3 train_kernel.py -d $domain -k $k -l $l -p $p -r $r -m $m --model-save-file ${SAVE_FILE} '>' $LOG_DIR/${DESC}.log
                 python3 train_kernel.py -d $domain -k $k -l $l -p $p -r $r -m $m --model-save-file ${SAVE_FILE} > $LOG_DIR/${DESC}.log
        fi
    done
done
