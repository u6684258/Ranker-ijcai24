...
def work(cmd, intermediate_file, log_file, timeout):
    timeouted = False
    wrong = False
    st = time.time()
    ...
    if not timeouted and 'Search stopped without finding a solution.' in out_text:
        ...
        print('success')
        # print(out_text)
        out_text = out_text.split("Running search")[1]
        plan_length = out_text.split("Plan length: ")[1].split("step(s).")[0]
        print(f"plan length: {plan_length}")
        expansions = out_text.split("Expanded ")[1].split("state(s).")[0]
        print(f"expansions: {expansions}")
        heuristic_calls = out_text.split("Evaluated ")[1].split("state(s).")[0]
        print(f"heuristic calls: {heuristic_calls}")
...
def domain_test(domain, test_file, model_file, mode="val", timeout=600, log_root=exp_root, model_type="gnn"):
...
if __name__ == "__main__":
    domain_test('blocks', 'val', 'test-hgn-ranker.dt', model_type="ranker")
    # single_test("ferry", "train", "p-l2-c10-s1.pddl", 'rank-ferry-L4-coord.dt')
    # single_test("blocks", "test", "p-l2-c10-s1.pddl", 'rank-ferry-L4-coord.dt')
