            ...
            for itr in range(self.iterations):
                ...
                for subset in subsets:
                    ...
                    for x in neighbour_nodes:
                        subset1 = tuple(sorted((u, v, x)))  # tuple(sorted(.)) is a hashable
                        subset2 = tuple(sorted((u, x, w)))  # tuple(sorted(.)) is a hashable
                        subset3 = tuple(sorted((x, v, w)))  # tuple(sorted(.)) is a hashable
                        colour = tuple(sorted((cur_colours[subset1], cur_colours[subset2], cur_colours[subset3])))
