...
def _unsorted_segment_helper(
    ...
    if len(segment_ids.shape) == 1:
        ...
        repeated_data = data.repeat_interleave(repeats, dim=0).to(data.device)
        ...
        indices = segment_ids[segment_ids != -1].to(data.device)
        assert repeated_data.shape[0] == indices.shape[0]

    # Placeholder for the segments
    segments = torch.zeros((num_segments, repeated_data.shape[1])).to(data.device)
...
def torch_unsorted_segment_sum(
    ...
    repeated_data = repeated_data.to(data.device)
    indices = indices.to(data.device)
    segments = segments.to(data.device)
