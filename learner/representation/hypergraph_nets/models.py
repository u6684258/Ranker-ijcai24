...
from representation.hypergraph_nets.modules import GraphIndependent, GraphNetwork
from representation.hypergraph_nets.utils import concat
...
def make_mlp_model(input_dim, hidden_units=32, num_layers=2, relu=True):
...
class MLPGraphIndependent(nn.Module):
    ...
    def __init__(
        ...
        self._gn = GraphIndependent(
            make_mlp_model(edge_input_size, hidden_units, relu=relu)
            if edge_input_size
            else _identity_function,
            make_mlp_model(node_input_size, hidden_units, relu=relu)
            if node_input_size
            else _identity_function,
            make_mlp_model(global_input_size, hidden_units, relu=relu)
...
class EncodeProcessDecode(nn.Module):
    ...
    def __init__(
        ...
        if global_output_size:
            ...
            self.global_model = _none_function
...
class StripsHGNNoReLU(EncodeProcessDecode):
    """
    Remove ReLU Layers for better performance
    """
    ...
    def __init__(
        ...
        super(StripsHGNNoReLU, self).__init__(receiver_k,
                                              sender_k,
                                              hidden_size,
                                              edge_input_size,
                                              edge_output_size,
    ...
    def reshape_output(self, shape):
        self.global_output_size = shape
        self.global_model = (
            nn.Sequential(
                nn.Linear(self.hidden_size, self.global_output_size),
                nn.ReLU(inplace=True),
            )
            if self.global_output_size
            else _none_function
        )
        self._output_transform = GraphIndependent(
            self.edge_model, self.node_model, self.global_model
