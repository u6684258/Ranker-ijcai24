...
for wl, iterations, prune, rep, domain, model in _CONFIGS:
    ...
    submit_cmd = f'sbatch --job-name=tr_{desc} --output={log_file} {SLURM_SCRIPT} "python3 train_kernel.py -m {model} -r {rep} -d {domain} -k {wl} -l {iterations} -p {prune} --model-save-file {model_save_file}"'
