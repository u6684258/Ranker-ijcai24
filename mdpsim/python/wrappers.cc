...
const py::list PyGroundAction::preconditions() const {
  py::list rv;
  AtomSet ignore_set;
  AtomList all_atoms;
...
const py::list PyGroundAction::add_effects() const {
  py::list rv;

  AtomList adds;
  AtomList deletes;
  UpdateList updates;
  action->effect().state_change(adds, deletes, updates, problem->terms(), problem->init_atoms(), problem->init_values());

  PyProblem pprob = PyProblem(problem);
  for (auto i = adds.cbegin(); i != adds.cend(); i++) {
    rv.append(PyProposition(*i, &problem->domain(),
                                  pprob.is_goal_atom(*i)));
  }
